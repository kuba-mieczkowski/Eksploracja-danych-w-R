  ...
  navbarPage("My Application",
             ...
             sidebarLayout(
              sidebarPanel(
               ...
               mainPanel(tableOutput("japanTable"),
...
server <- function(input, output) {
    ...
    output$india1Plot <- renderPlot({nr_heatwaves <- data.frame(
      heat = c(100,	93,	90,	94, NA,	279,	40,	201,	100,	144,	86,	150,	123	,90,	174,	42,	36),
      year = c('1970-79',	'1980-89',	'1990-99',	'2000-09'	,'','2010',	'2011',	'2012',	'2013',	'2014',	'2015',	'2016',	'2017',	'2018',	'2019',	'2020',	'2021'
      )
    ) 
    nr_heatwaves %>% 
      ggplot(aes(x = forcats::fct_inorder(factor(year)), y = heat)) + geom_bar(stat = 'identity', color = 'orange', fill = 'orange') + 
    ...
    output$india2Plot <- renderPlot({
      ...
      nr_deaths <- data.frame(
        year = c('1970-79',	'1980-89',	'1990-99',	'2000-09'	,'2010 - 2019'),
        deaths = c(2488,1505,2916,4056,6496)
      )
      nr_deaths %>% 
        ggplot(aes(x = year, y = deaths)) + geom_bar(stat = 'identity', color = 'darkolivegreen', fill = 'darkolivegreen') +
    ...
    output$dominikanaPlot <- renderPlotly({
      ...
      fig <- plot_ly(dominikana_wody,x = ~Year, y = ~RCP_2.6, colors = 'Paired', name = 'RCP 2.6',type = 'scatter', mode = 'lines')
      fig <- fig %>% 
        add_trace(y = ~RCP_2.6_low, name = 'RCP 2.6 10-90th Percentile Range (low)', mode = 'lines + markers') %>% 
        add_trace(y = ~RCP_2.6_high, name = 'RCP 2.6 10-90th Percentile Range (high)', mode = 'lines + markers') %>% 
